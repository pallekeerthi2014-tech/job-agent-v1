...
@router.post("/admin/gmail/mailboxes/{mailbox_id}/forwarding-active", response_model=CandidateMailboxRead)
def mark_candidate_mailbox_forwarding_active(
    mailbox_id: int,
    db: Session = Depends(get_db),
    _: User = Depends(require_super_admin),
) -> CandidateMailboxRead:
...
def gmail_oauth_url(
    ...
    return GmailOAuthUrlResponse(
        candidate_id=candidate_id,
        authorization_url=build_candidate_oauth_url(candidate_id=candidate_id),
    )
